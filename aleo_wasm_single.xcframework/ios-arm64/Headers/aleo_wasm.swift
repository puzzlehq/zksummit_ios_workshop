...
private extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_aleo_wasm_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_aleo_wasm_rustbuffer_free(self, $0) }
    }
}
...
private func uniffiInitForeignExecutor() {
    ffi_aleo_wasm_foreign_executor_callback_set(uniffiForeignExecutorCallback)
}
...
private func uniffiFutureCallbackHandlerStringString(
    rawContinutation: UnsafeRawPointer,
    returnValue: RustBuffer,
    callStatus: RustCallStatus
) {
    ...
    do {
        try uniffiCheckCallStatus(callStatus: callStatus, errorHandler: FfiConverterString.lift)
        try continuation.pointee.resume(returning: FfiConverterString.lift(returnValue))
    } catch {
        continuation.pointee.resume(throwing: error)
    }
...
public func computeSerialNumber(plaintextRecord: String, privateKeyString: String, programName: String, recordName: String) -> String {
    return try! FfiConverterString.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_compute_serial_number(
                FfiConverterString.lower(plaintextRecord),
                FfiConverterString.lower(privateKeyString),
                FfiConverterString.lower(programName),
...
public func decryptCiphertextRecord(inputRecordCiphertext: String, viewKey: String) -> String {
    return try! FfiConverterString.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_decrypt_ciphertext_record(
                FfiConverterString.lower(inputRecordCiphertext),
...
public func decryptTransitionCiphertext(viewKey: String, ciphertext: String, tpk: String, programName: String, functionName: String, index: UInt16) -> String {
    return try! FfiConverterString.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_decrypt_transition_ciphertext(
...
public func executeProgram(privateKeyString: String, program: String, function: String, inputs: [String], feeCredits: Double, feeRecord: String, url: String, cache: Bool) async throws -> String {
    ...
    return try await withCheckedThrowingContinuation {
        continuation = $0
        try! rustCall {
            uniffi_aleo_wasm_fn_func_execute_program(
                FfiConverterString.lower(privateKeyString),
                ...
                FfiConverterString.lower(feeRecord),
                FfiConverterString.lower(url),
                FfiConverterBool.lower(cache),
                FfiConverterForeignExecutor.lower(UniFfiForeignExecutor()),
                uniffiFutureCallbackHandlerStringString,
                &continuation,
                $0
            )
        }
...
public func newPrivateKeyString() -> String {
    return try! FfiConverterString.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_new_private_key_string($0)
        }
    )
}

public func parseMicrocredits(plaintextRecord: String) -> UInt64 {
    return try! FfiConverterUInt64.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_parse_microcredits(
                FfiConverterString.lower(plaintextRecord), $0
            )
        }
    )
}

public func privateKeyToAddress(input: String) -> String {
    return try! FfiConverterString.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_private_key_to_address(
...
public func privateKeyToViewKey(input: String) -> String {
    return try! FfiConverterString.lift(
        try! rustCall {
            uniffi_aleo_wasm_fn_func_private_key_to_view_key(
                FfiConverterString.lower(input), $0
            )
        }
    )
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_aleo_wasm_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if uniffi_aleo_wasm_checksum_func_compute_serial_number() != 2030 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_decrypt_ciphertext_record() != 53008 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_decrypt_transition_ciphertext() != 22278 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_execute_program() != 54356 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_new_private_key_string() != 13563 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_parse_microcredits() != 20167 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_private_key_to_address() != 16959 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_aleo_wasm_checksum_func_private_key_to_view_key() != 45693 {
        return InitializationResult.apiChecksumMismatch
    }
